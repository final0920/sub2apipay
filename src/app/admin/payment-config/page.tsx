...
function getTexts(locale: Locale) {
  return locale === 'en'
    ? {
        ...
        providerManagementHint: 'Add and manage payment provider instances. Each instance can be independently configured and enabled.',
        ...
        noInstancesHint: 'Click "Add Instance" to configure a payment provider',
        ...
        basicConfigHint: 'Recharge and order related settings',
        ...
        // Override env section
        overrideEnvConfig: 'Override Env Config',
        overrideEnvHint: 'When enabled, database settings override environment variables for the following fields',
    ...
    : {
        ...
        providerManagementHint: '添加和管理支付服务商实例，每个实例独立配置和启停',
        addInstance: '添加实例',
        editInstance: '编辑实例',
        instanceName: '实例名称',
        ...
        noInstances: '暂无实例',
        noInstancesHint: '点击「添加实例」配置支付服务商',
        ...
        overrideEnvHint: '开启后，以下字段将使用数据库配置覆盖环境变量',
...
function PaymentConfigContent() {
  ...
  const openCreateInstance = (providerKey?: string) => {
    setEditingInstance(null);
    setInstanceForm({
      providerKey: providerKey || 'easypay',
      name: '',
      enabled: true,
      sortOrder: 0,
      config: {},
      supportedTypes: PROVIDER_SUPPORTED_TYPES[providerKey || 'easypay'] || [],
    });
    setError('');
    setInstanceModalOpen(true);
  };
  ...
  return (
    ...
    >
      ...
      {/* ═══════════════════════════════════════════════
          Section 1: Provider Management (大卡片)
          ═══════════════════════════════════════════════ */}
      <div className={`${cardCls} mb-4`}>
        ...
        {Object.entries(PROVIDER_LABELS).map(([providerKey, labels]) => {
          const providerInstances = instancesByProvider[providerKey] || [];
          const providerLabel = labels[locale];
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <span
                      className={[
                        'inline-block h-3.5 w-3.5 rounded-full bg-white transition-transform',
                        instanceForm.enabled ? 'translate-x-4.5' : 'translate-x-0.5',
                      ].join(' ')}
                    />
                  </button>
